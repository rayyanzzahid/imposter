    ...
    <main className="flex min-h-screen flex-col items-center gap-6 px-6 py-12">
      <div className="text-center">
        ...
        <div className="flex items-center gap-3 justify-center">
          <h1 className="text-4xl font-black tracking-widest text-neon-purple">
            {room.code}
          </h1>
          <button
            onClick={handleCopyCode}
            className="rounded-lg bg-surface border border-white/10 px-3 py-2 text-sm text-white"
          >
            {copied ? '✓' : 'Copy'}
          </button>
        </div>
      ...
      <div className="w-full max-w-sm flex flex-col gap-3 mt-4">
        {isHost && (
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl bg-surface px-4 py-3 text-white border border-white/10 outline-none"
            >
              {CATEGORIES.map((c) => (
            ...
            <div className="mt-3">
              <label className="text-zinc-400 text-sm mb-1 block">Number of Rounds</label>
              <select
                value={totalRounds}
        ...
        {isHost && (
          <button
            ...
            className="rounded-2xl bg-neon-purple px-6 py-4 font-bold text-white disabled:opacity-40"
          >
            {starting ? 'Starting...' : 'Start Game'}
          </button>
        )}

        <button onClick={handleLeave} className="text-zinc-400 text-sm">
          Leave Room
        </button>
      </div>
    </main>
